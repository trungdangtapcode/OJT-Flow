...
| Retrieval operators can compare recent search runs | `tests/test_frontend_architecture.py::test_retrieval_page_surfaces_runtime_ranking_stack`, `docs/frontend_architecture.md`, `/api/v1/retrieval/judgments`, `/api/v1/retrieval/judgments/summary`, `/api/v1/retrieval/judgments/evaluate`, `knowledge/retrieval/evaluation_policy.json`, and Retrieval UI in-session run history, selectable baseline, durable explicit relevance judgments, judgment-aware metrics, server HitRate/MAP/MRR/nDCG evaluation, policy-driven evaluation recommendations, stored-label summary, copyable `retrieval_judgment_evaluation` report with rule-pack fingerprints, overlap/churn metrics, delta, rank movement, rule-pack fingerprint comparison, copyable comparison report, and evidence add/remove/retain rendering in `frontend/src/features/retrieval/retrieval-page.tsx`. |
