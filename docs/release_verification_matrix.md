...
| Retrieval operators can compare recent search runs | `tests/test_frontend_architecture.py::test_retrieval_page_surfaces_runtime_ranking_stack`, `docs/frontend_architecture.md`, `/api/v1/retrieval/judgments`, `/api/v1/retrieval/judgments/summary`, and Retrieval UI in-session run history, selectable baseline, durable explicit relevance judgments, judgment-aware metrics, stored-label summary, overlap/churn metrics, delta, rank movement, copyable JSON report, and evidence add/remove/retain rendering in `frontend/src/features/retrieval/retrieval-page.tsx`. |
