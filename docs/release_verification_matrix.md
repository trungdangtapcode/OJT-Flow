...
| Retrieval query analysis reports data-driven query-quality diagnostics | `tests/test_retrieval.py::test_query_analysis_reports_quality_diagnostics`, `tests/test_retrieval.py::test_query_analysis_uses_data_driven_diagnostic_rules`, `tests/test_retrieval.py::test_query_analysis_reports_conflicting_standard_filter`, `tests/test_retrieval.py::test_query_analysis_detects_medication_and_analytics_routes`, `knowledge/retrieval/query_diagnostic_rules.json`, `docs/retrieval_module_v0.md`, `OJT_QUERY_DIAGNOSTIC_RULES_PATH`, and Retrieval UI diagnostic rendering in `frontend/src/features/retrieval/retrieval-page.tsx`. |
...
| Retrieval packages expose package-level quality signals | `tests/test_retrieval.py::test_retrieval_quality_signals_flag_missing_standard_coverage`, `tests/test_retrieval.py::test_retrieval_quality_summary_uses_data_driven_policy`, `tests/test_retrieval.py::test_retrieval_quality_signals_flag_weak_top_hit_match`, `tests/test_retrieval.py::test_retrieval_quality_signals_flag_weak_evidence_provenance`, `tests/test_retrieval.py::test_retrieval_quality_signals_flag_missing_concept_grounding`, `tests/test_frontend_architecture.py::test_retrieval_page_surfaces_runtime_ranking_stack`, `docs/retrieval_module_v0.md`, `knowledge/retrieval/quality_gate_policy.json`, `quality_signals[]`, `quality_signals[].metadata`, `quality_summary`, `handoff_context.quality_policy`, `handoff_context.quality_summary`, policy-driven `ranking_thresholds.min_top_matched_terms`, policy-driven `provenance_requirements`, policy-driven `concept_grounding_requirements`, `source_locator.concept_matches[]`, and Retrieval UI quality checklist, metadata signal details, plus readiness-score/policy rendering in `frontend/src/features/retrieval/retrieval-page.tsx`. |
| Retrieval operators can compare recent search runs | `tests/test_frontend_architecture.py::test_retrieval_page_surfaces_runtime_ranking_stack`, `docs/frontend_architecture.md`, `/api/v1/retrieval/judgments`, `/api/v1/retrieval/judgments/summary`, `/api/v1/retrieval/judgments/evaluate`, `knowledge/retrieval/evaluation_policy.json`, and Retrieval UI in-session run history, query-profile summaries, selectable baseline, durable explicit relevance judgments, judgment-aware metrics, server HitRate/MAP/MRR/nDCG evaluation, policy-driven evaluation recommendations, stored-label summary, copyable `retrieval_judgment_evaluation` report with query profile and rule-pack fingerprints, overlap/churn metrics, delta, rank movement, compact comparison diagnosis, visible query-profile comparison, concept-grounding comparison, query-aspect comparison, standard/aspect coverage comparison, quality-signal comparison, selected-hit facet coverage comparison, rule-pack fingerprint comparison, copyable comparison report, and evidence add/remove/retain rendering in `frontend/src/features/retrieval/retrieval-page.tsx`. |
| Retrieval has deterministic quality gates for known healthcare evidence queries | `tests/test_retrieval.py::test_retrieval_eval_fixture_passes_static_repository`, `tests/test_retrieval.py::test_retrieval_eval_cli_outputs_json_summary`, graded judgments in `tests/fixtures/retrieval_eval_cases.json`, and MAP/NDCG/Coverage reporting in `scripts/evaluate-retrieval.py`. |
