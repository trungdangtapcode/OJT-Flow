...
| Retrieval operators can compare recent search runs | `tests/test_frontend_architecture.py::test_retrieval_page_surfaces_runtime_ranking_stack`, `docs/frontend_architecture.md`, and Retrieval UI in-session run history, delta, rank movement, and evidence add/remove/retain rendering in `frontend/src/features/retrieval/retrieval-page.tsx`. |
