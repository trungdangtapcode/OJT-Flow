...
- failed extraction is persisted as a failed workflow with audit events.
...
| --- | --- | --- |
| `OJT_MAX_UPLOAD_BYTES` | `26214400` | Maximum uploaded file size in bytes. |
| `OJT_UPLOAD_READ_CHUNK_BYTES` | `1048576` | Read chunk size for multipart uploads. |
| `OJT_ALLOWED_UPLOAD_EXTENSIONS` | Built-in allowlist | Comma-separated extension allowlist. Values may include or omit the leading dot. |
