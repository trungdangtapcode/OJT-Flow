...
| 2026-06-10 | F006 | Started | Added typed storage consistency contracts, reusable workflow artifact scanner, `/api/v1/runtime/storage-consistency`, readiness integration, dataset metadata listing across memory/SQLite/Postgres, workflow-ref-to-dataset-row checks, dataset-row-to-file checks, dataset hash checks, and unreferenced dataset row candidate counts. Remaining work: knowledge chunk/source ref scan and admin report. |
...
| 2026-06-11 | F015 | Started | Frontend requests now send `X-Request-ID`, backend responses echo it, error envelopes include `error.request_id` and `details.request_id`, unhandled API logs include request ID, assistant stream events/replays carry the request ID, and retrieval reindex background job records store the originating request ID. Remaining work: persist request IDs into workflow audit events and retrieval traces. |
...
| 2026-06-11 | F019 | Started | Added `OJT_NO_MOCK_DATA` and effective no-mock policy in pilot/production modes. Remaining work: audit every demo fixture/starter-data path and block or replace fixture data when effective no-mock is enabled. |
...
- [ ] F006 Add a database consistency checker for workflow JSON refs, dataset refs, output refs, knowledge chunk refs, and missing local files.
...
- [ ] F015 Add request correlation IDs from frontend request to backend logs, workflow events, assistant tool calls, and retrieval traces. _(Started: API/frontend/error/assistant stream/background-job propagation is implemented.)_
...
- [ ] F019 Add a no-mock runtime policy flag that blocks demo fixtures from appearing in production-like modes.
...
- [ ] F021 Make upload parsing asynchronous for large PDFs, images, spreadsheets, and multi-file batches.
- [ ] F022 Add a first-class `UploadedArtifact` contract with file hash, MIME type, extension, byte size, source, user, and retention policy.
- [ ] F023 Add deduplication by content hash for uploaded documents and extracted text.
- [ ] F024 Add a parsing pipeline trace with extractor chosen, fallback path, warnings, token/character counts, and confidence.
- [ ] F025 Add MarkItDown extraction as one extractor option behind a common `DocumentExtractor` port.
...
- [ ] F077 Add OpenAI embedding mode and local Hugging Face embedding mode with dimension compatibility checks and reindex requirements.
- [ ] F078 Add vector index generation IDs so retrieval can detect stale embeddings after provider/model changes.
- [ ] F079 Add pgvector HNSW/IVFFlat index management docs and migration path for large corpora.
...
- [ ] F096 Make Assistant the default end-user landing page with task starters and file drop zone.
- [ ] F097 Add persistent ChatGPT-like sessions backed by Postgres, with backend-owned title generation.
