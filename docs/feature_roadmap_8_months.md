...
- Rule-based parser, validator, converter, FHIR-like profiler, OCR evidence
...
| 2026-06-11 | F006 | Implemented | Added typed storage consistency contracts, reusable workflow artifact scanner, `/api/v1/runtime/storage-consistency`, readiness integration, dataset metadata listing across memory/Postgres, workflow-ref-to-dataset-row checks, dataset-row-to-file checks, dataset hash checks, unreferenced dataset row candidate counts, and retrieval knowledge/index consistency counters for seeded/corpus scopes. |
...
| 2026-06-11 | F010 | Implemented | Added typed Assistant session/message contracts, explicit workflow refs, auto-extraction of nested workflow IDs from tool payloads, `AssistantSessionService`, memory/Postgres repositories, Postgres migration 008, authenticated session/message API routes, frontend session sidebar loading, automatic user/assistant message persistence, and stream-event replay payloads. |
...
| 2026-06-11 | F013 | Implemented | Assistant stream requests now accept `session_id`, stamp SSE events with stream/session/sequence metadata, persist one replay artifact per stream in dedicated memory/Postgres storage, expose `/api/v1/assistant/sessions/{session_id}/stream-replays`, and keep replay artifacts out of the normal chat transcript. |
...
| 2026-06-11 | F008 | Implemented | Added typed `BackgroundJob` contracts, memory/Postgres repositories, Postgres migration 009, job status/progress/error JSON, owner-scoped job listing, and durable metadata for retrieval reindex, file parse, OCR, embedding reindex, external ingestion, and export package jobs. |
| 2026-06-11 | F009 | Implemented | Added `BackgroundJobService`, durable Postgres job records, sync local runner mode, and RabbitMQ/Celery queue-backed workers for long-running parse/OCR/RAG-style operational jobs. Runtime readiness reports whether the active runner is sync-local or queue-backed. |
...
| 2026-06-11 | F022 | Implemented | Added `UploadedArtifact` contracts, owner-scoped artifact repositories for memory/Postgres, raw upload storage under `var/uploads/`, artifact list/get APIs, and Postgres migration 011. |
...
| 2026-06-11 | F037 | Implemented | Added rule-based extraction quality scoring based on empty/short text, low extractor confidence, extractor warnings, PDF OCR requirement, and workbook structure warnings. |
...
| 2026-06-11 | F033 | Implemented | Added rule-based redaction preview with structured CSV sensitive-column masking, SSN/email/phone span detection, `POST /api/v1/parse/redaction-preview`, and trace-level redaction summaries before external provider handoff. |
...
| 2026-06-11 | F035 | Implemented | Added owner-scoped artifact download, metadata export, and access-event listing APIs backed by append-only memory/Postgres artifact access logs and Postgres migration 012. |
...
| 2026-06-11 | F084 | Implemented | Added `RetrievalPackage.support_matrix` and `handoff_context.support_matrix`, giving retrieval/assistant/MCP clients a rule-based claim-to-evidence matrix with source IDs, locators, matched terms, scores, reasoning, warnings, and support status. |
...
| 2026-06-11 | F102 | Implemented | Added failed Assistant tool-call recovery actions: inline Retry resubmits the exact failed tool name and original arguments through structured `assistant_recovery.retry_tool`, while Continue creates a rule-based no-tool continuation that keeps unresolved failures visible. |
...
| 2026-06-11 | F107 | Implemented | Added data-driven Assistant memory policy in `knowledge/assistant/memory_policy.json`, typed memory contracts, memory/Postgres repositories, Postgres migration 014, authenticated memory policy/snapshot/upsert/delete APIs, chat context injection that strips caller-spoofed memory, an MCP memory-policy resource, and an Advanced context UI editor for policy-allowed operational preferences only. |
...
| 2026-06-11 | F052 | Implemented | Added review-gated `TerminologyCandidate` contract and rule-based LOINC candidate generation for `lab_result_v1.lab_name` from `knowledge/terminologies/medical_concepts.json`, carried in `ClinicalPackage.terminology_candidates`. |
...
| 2026-06-11 | F116 | Implemented | Added durable generic audit records for Assistant and MCP tool calls, with memory/Postgres repositories, Postgres migration 015, owner-scoped `GET /api/v1/audit/records`, redacted tool input/output hashes, request/session/workflow/workflow-event correlation, and docs/tests. |
...
| 2026-06-11 | F117 | Implemented | Added data-driven Assistant evaluation fixtures at `knowledge/assistant/evaluation_cases.json`, typed suite/case contracts, fixture loader, evaluation docs, and rule-based tests for tool selection, write-gate preservation, answer terms, and evidence-source requirements. |
...
| 2026-06-11 | F119 | Implemented | Added organization/workspace governance contracts, data-driven workspace defaults, memory/Postgres repositories, Postgres migration 016, authenticated organization APIs, workspace settings merge/versioning, group creation, and docs/tests. RBAC enforcement and cross-resource ownership checks remain scoped to F120/F121. |
...
| 2026-06-11 | F122 | Implemented | Added service-account identity for automation with `identity_type=service_account`, one-time `ojt_sa_` bearer token issuance, configurable token TTL/default role, memory/Postgres storage, Postgres migration 017, workspace membership assignment with explicit RBAC role, authenticated list/create APIs, service-account-aware `/auth/me`, docs, and focused API/unit tests. |
...
| 2026-06-11 | F124 | Implemented | Added rule-based PHI redaction policy actions for masking, suppression, token placeholders, and review-gated reveal, with redaction preview API support and external-provider block recommendations. |
...
| 2026-06-11 | F128 | Implemented | Added generated-output validation contracts and policy helpers for Assistant plans, summaries, streamed summaries, and export descriptions; Assistant now validates LLM plans before execution/display and fails with structured errors when generated summaries fail validation. |
...
| 2026-06-11 | F130 | Implemented | Added rule-based audit hash-chain fields, repository-level chain linking for memory/Postgres audit records, Postgres migration 018, high-risk deployment config signal, runtime config exposure, docs, and focused persistence tests. |
...
| 2026-06-11 | F083, F095 | Implemented | Added rule-based retrieval answer synthesis backed only by support matrices, ranked evidence, citations, and Graph-NER path refs. Unsupported evidence now produces explicit refusal or gaps, and stale/deprecated/review-needed/unapproved/version-mismatched sources produce first-class freshness warnings. |
...
| 2026-06-11 | F089, F092 | Implemented | Added persisted Graph-NER context records for retrieval packages, with owner/workflow/request/search metadata, memory/Postgres repositories, Postgres migration 019, owner-scoped graph listing and JSONL/RDF-like export APIs, API docs, and focused restart/API tests. |
...
| 2026-06-12 | F187 | Implemented | Added PHI-safe private corpus ingestion: `POST /api/v1/retrieval/private-corpus/ingest`, inline or uploaded-artifact text sources, rule-based redaction preview before indexing, redacted-text-only private chunks, retention policy stamping, external-provider blocking metadata, tenant-scoped source inventory/search, frontend API/server-state boundary, docs, and focused tests. |
| 2026-06-12 | F188 | Implemented | Added external medical-search transparency records on retrieval packages for PubMed, ClinicalTrials.gov, and openFDA hints, including exact query/URL, parsed parameters, filters, result IDs placeholder, cache state/key/policy metadata, source release placeholder, rate-limit/auth metadata, execution status, handoff context wiring, docs, and focused package tests. |
| 2026-06-12 | F178 | Implemented | Added approval-gated embedding reindex safety workflow with `GET /api/v1/retrieval/embedding-reindex/dry-run`, `POST /api/v1/jobs/embedding-reindex`, rule-based approval tokens, sanitized rollback markers, post-run manifest/quality comparison, docs, and focused tests. |
...
| 2026-06-12 | F184 | Implemented | Expanded durable retrieval judgments from relevance-only labels to reviewer workflow labels: relevant, partial, irrelevant, unsafe, stale, and source-policy-blocked; updated API/contracts, Postgres migration 020, UI controls, metrics/report counts, docs, and focused API/service/frontend build checks. |
| 2026-06-12 | F185 | Implemented | Added durable retrieval active-learning candidates with memory/Postgres repositories, Postgres migration 021, API list/summary/enqueue/update routes, automatic queueing from reviewer judgments and low-confidence evaluations, Retrieval UI queue surfacing, docs, and focused service/API checks. |
...
5. Query planning: the router classifies intent, clinical domain, safety risk,
   ...
   strategy.
6. Retrieval and reranking: semantic vector search collects candidates, applies
   metadata filters, optionally reranks with a configured cross-encoder, and
   fails that model-backed stage if the provider is unavailable.
...
- Cross-encoder reranking in local GPU mode, with explicit unavailable/error
  state when the reranker cannot run.
...
- [x] F076 Add cross-encoder reranking for local GPU mode with a configurable model, candidate limit, and explicit unavailable state.
...
- [x] F187 Add PHI-safe private corpus ingestion with redaction preview, retention policy stamping, and external-provider blocking by default.
- [x] F188 Add PubMed/ClinicalTrials/openFDA query transparency records showing exact external query, filters, result IDs, cache hit state, and rate-limit metadata.
...
- [x] F158 Add observability dashboard with API metrics, job metrics, retrieval metrics, LLM cost, error rate, and queue depth. _(Implemented: Prometheus/Grafana, RabbitMQ exporter, Celery exporter, Node Exporter, Loki/Promtail, and Sentry DSN hooks are wired for the Compose stack.)_
