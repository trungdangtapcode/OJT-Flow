...
Manual data conversion is error-prone and difficult to audit. & Use rule-based tools, schema validators, FHIR/OMOP constraints, versioned workflows, and audit logs. \\
...
\addlinespace
Phase 2 & Week 2 & Implement rule-based parsing, JSON/YAML/CSV conversion, schema validation, workflow state, MCP tool contracts, basic agent orchestration, and unit tests. & Core workflow MVP. \\
\addlinespace
...
\begin{figure}[H]
  ...
  \caption{Architecture overview for OJTFlow. The model reasons and explains, while MCP-backed rule-based services parse, validate, transform, retrieve, review, and audit.}
\end{figure}
...
\begin{enumerate}
  ...
  \item Transformation Agent performs rule-based conversion or cleaning.
...
Transformation Module & Converts and cleans data through rule-based tools, never uncontrolled model generation. & Converted output, diff, metadata. \\
